...
public:
	...
	int32			recvfrom(void* pBuf, int nLen, InterAddress& addFrom, int nFlag = 0);
	int32			recvfrom(void* pBuf, int nLen, InterAddress& addFrom, const TimeValue& tmVal, int nFlag = 0);

	int32			sendto(const void *pBuf, const int nLen, const InterAddress& addrTo, int nFlag = 0);
	int32			sendto(const void *pBuf, const int nLen, const InterAddress& addrTo, const TimeValue& tmVal, int nFlag = 0);
protected:
